...
class LearnedIndex:
    ...
    def build(self):
        """
         Build an index for data and train a regression model.

         This function creates an index that maps labels to data, and then trains a regression model
         using the data and labels provided during object initialization.

         The resulting index is stored in the 'index' attribute, and the trained regression model
        ...
        if self.reg == Regression.LINEAR:
            ...
            self.X = poly.fit_transform(self.keys.reshape(-1, 1))
            self.Y = self.labels.reshape(-1, 1)
            self.model = LinearRegression()
            self.model.fit(self.X, self.Y)
    ...
    def find_all(self):
        ...
        for i in range(len(self.data)):
            pred, err = self.find(self.data[i])
            stats[err] += 1
            predictions.append(pred)
        scatter_plot(self.keys.reshape(-1, 1), self.Y, predictions, "Scatter")
        return stats
